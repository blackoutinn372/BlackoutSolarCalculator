...
public class CalculationClient implements EntryPoint 
{
	...
	private Widget loadAllControlsNew() {
		...
		txtSecondBank.addMouseOutHandler(new txtSecondBankValidation());
	...
	class txtSecondBankValidation implements MouseOutHandler{
		@Override
		public void onMouseOut(MouseOutEvent event) {
			String validFormat ="^([-+] ?)?[0-9]+(,[0-9]+)?$";
	...
	private void loadAllUIControls() {
		...
		btnCalculation.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				{
					Window.alert("Please enter digit numbers only for household size.");
					return;
				}
				doCalculation();
